...
;(function($){
    $.fn.ylColor = function(options){
        ...
        settings.containerId = settings.editorId +"-"+ settings.containerId;
        var elems = this;
        var cur_elem;
        ...
        var event = function(){
            ...
            $('#' + settings.containerId).find('.aCol').unbind("click");
            $('#' + settings.containerId).find('.aCol').bind('click',function(){
                onSelect(this);
            })
            $('.close-col-modal').bind('click',function(){
            	hide();
            })
        };
        var build = function(){
        	...
        	if($('#' + containerId).length == 0){
	            ...
	            customDiv.append('<div class="title">主题颜色<span class="close-col-modal">X<span></div><ul>'+liHtml+'</ul>');
	
	            liHtml = '';
	            $.each(settings.formatColor,function(i,e){
	                liHtml += '<li class="aCol" data-color="'+e+'" style="background-color: '+e+'" title="'+ e +'"></li>';
	            });
	            customDiv.append('<div class="title">标准颜色</div><ul>'+liHtml+'</ul>');
